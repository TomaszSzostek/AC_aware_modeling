...
frag_plot_data <- fragments_df %>%
  ...
  mutate(
    rank = row_number(),
    status = case_when(
      was_removed_by_AC == TRUE ~ "CAFE Removed",
      selection_method == "AC_added" ~ "CAFE Added",
      TRUE ~ "Other"
    ),
...
ggsave("R_reporting/figures/Figure_4.pdf", final_fig, width = 14, height = 11, dpi = 500, device = cairo_pdf)
ggsave("R_reporting/figures/Figure_4.png", final_fig, width = 14, height = 11, dpi = 500, bg = "white")
