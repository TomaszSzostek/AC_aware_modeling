################################################################################
# Figure 3: CAFE LATE Scoring Analysis
################################################################################

cat("\n", strrep("=", 80), "\n")
cat("Figure 3: CAFE LATE Analysis\n")
cat(strrep("=", 80), "\n\n")
...
panel_a <- ggplot(cafe_molecules, aes(x = cafe_boost, color = enrichment_category, fill = enrichment_category)) +
  ...
  theme(
    ...
    legend.title = element_text(size = 28, face = "bold"),  # Increased
    legend.text = element_text(size = 24, face = "bold"),  # Increased
    legend.key.size = unit(1.2, "cm"),
    legend.spacing.y = unit(0.3, "cm"),
    axis.text.x = element_text(size = 24),  # Increased
    axis.text.y = element_text(size = 24),  # Increased
    axis.title.x = element_text(size = 28, face = "bold"),  # Increased
    axis.title.y = element_text(size = 28, face = "bold"),  # Increased
...
panel_b <- ggplot(frag_data, aes(x = reorder(fragment_label, ac_enrichment), y = ac_enrichment)) +
  ...
  geom_text(aes(label = paste0("n=", n_cliff_pairs)), 
            vjust = -2.8, size = 10, fontface = "bold",  # Increased
  ...
  theme(
    ...
    axis.text.y = element_text(size = 28, color = "gray20"),  # Increased
    axis.title.y = element_text(size = 32, face = "bold", margin = margin(r = 15)),  # Increased
    ...
    legend.title = element_text(size = 24, face = "bold", hjust = 0.5),  # Increased
    legend.text = element_text(size = 20),  # Increased
    plot.margin = margin(20, 35, 140, 50)  # Increased left margin to shift panel right
  ) +
...
panel_c <- ggplot() +
  ...
  annotate("text", x = activity_threshold / 2, y = max_enrichment * 0.95, 
           label = "Inactive", size = 10, fontface = "bold", color = "#c62828") +  # Increased
  annotate("text", x = (activity_threshold + 1) / 2, y = max_enrichment * 0.95, 
           label = "Active", size = 10, fontface = "bold", color = "#2e7d32") +  # Increased
  ...
  # LARGE vertical colorbar replaces Y-axis - fitted to plot height, MUCH WIDER
  scale_color_gradient(
    ...
    guide = guide_colorbar(
      ...
      title.hjust = 0.5,
      title.vjust = 1,
      barwidth = 7.0,   # Slightly increased width
      barheight = 45.0,   # Slightly increased height
      label.theme = element_text(size = 25),  # Increased
      title.theme = element_text(size = 32, face = "bold")  # Increased
    )
  ) +
  ...
  # Legend for Rescued - right top corner, MUCH LARGER
  annotate("rect", xmin = 0.86, xmax = 0.995, 
           ymin = max_enrichment * 0.85, ymax = max_enrichment * 0.98,
           fill = "white", color = "gray80", linewidth = 0.3, alpha = 0.7) +
  annotate("point", x = 0.88, y = max_enrichment * 0.915, color = "#b3e5fc", shape = 17, size = 10, alpha = 0.7) +  # Increased
  annotate("text", x = 0.92, y = max_enrichment * 0.915, label = "Rescued", hjust = 0, vjust = 0.5, size = 10, color = "black", fontface = "bold") +  # Increased
  
  # Clean theme - minimal, publication-ready
  theme_bw() +
  theme(
    legend.position = "right",  # LARGE vertical colorbar on right side
    legend.justification = "center",
    legend.background = element_rect(fill = "white", color = "black", linewidth = 1.2),  # Frame like other legends
    legend.title = element_text(size = 36, face = "bold", color = "black", angle = 90),  # Increased
    legend.text = element_text(size = 22, color = "black"),  # Increased
    axis.text.x = element_text(size = 24, color = "black"),  # X-axis labels - Increased
    axis.text.y = element_blank(),  # No Y-axis numeric labels
    axis.ticks.y = element_blank(),  # No Y-axis ticks
    axis.title.x = element_text(size = 30, face = "bold", color = "black"),  # X-axis title - Increased
    ...
    plot.margin = margin(20, 90, 20, 50)  # Increased side margins to slightly reduce panel width
  )
...
final_fig <- plot_grid(
  ...
  label_size = 32,  # Increased
  label_fontface = "bold",
  rel_heights = c(1, 1.1)  # Restored original height
)
...
ggsave("R_reporting/figures/Figure_3.pdf", final_fig, width = 28, height = 20, dpi = 500, device = cairo_pdf)
ggsave("R_reporting/figures/Figure_3.png", final_fig, width = 28, height = 20, dpi = 500, bg = "white")

cat("\nFigure 3 saved (500 DPI, 28×20 inches)\n")
