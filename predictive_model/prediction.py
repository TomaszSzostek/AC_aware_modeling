"""
CliffAware QSAR Evaluation Module

A comprehensive QSAR evaluation framework for binary classification of molecular activity
with special focus on activity cliff (AC) prediction and cheminformatics gold standard practices.

Key Features:
- Multi-backbone feature engineering: RDKit 2D descriptors, Mordred descriptors, ECFP fingerprints
- Advanced feature selection using Boruta algorithm optimized for small datasets (≤350 compounds)
- Group-based data splitting to prevent data leakage (cliff-group or Butina clustering)
- Comprehensive model evaluation with 9 different ML algorithms
- Activity cliff-aware metrics and evaluation
- Publication-ready visualizations and statistical analysis
- Robust cross-validation and bootstrap confidence intervals
...
# Removed: format_metrics_with_combined_ci() - formatting is now done inline
...
def _rank_models(df_metrics: pd.DataFrame, primary: str, stability_weight: float = 1.0) -> pd.DataFrame:
    """
    Rank models by composite score balancing mean performance and stability.
    
    Ranking logic:
    1. Composite score = Mean + (stability_weight × CI_width)
       - Lower is better for RMSE/Brier/ECE
       - Higher is better for AUC/F1/MCC
    2. Tie-breakers: AUPRC_active, AUROC, Brier
    
    This approach balances predictive performance with reliability, which is crucial
    for production deployment in molecular generation applications.
    
    Args:
        df_metrics: DataFrame with model metrics
        primary: Primary metric name (e.g., "Cliff_RMSE")
        stability_weight: Weight (α) for stability penalty
            - α = 0.5: Prioritize mean performance
            - α = 1.0: Balanced (equal weights)
            - α = 2.0: Prioritize stability
        
    Returns:
        Sorted DataFrame (best model first by composite score)
    """
    prim_in = str(primary)
    ...
    # Compute composite score: Mean + (α × CI_width)
    mean_value = dfc[prim].astype(float)
    stability_penalty = stability_weight * dfc["_ci_width"]
    
    if minimize:
        # For metrics where lower is better (RMSE, Brier)
        dfc["_composite_score"] = mean_value + stability_penalty
    else:
        # For metrics where higher is better (AUC, F1)
        # Convert to minimization: -Mean + (α × CI_width)
        dfc["_composite_score"] = -mean_value + stability_penalty
    
    # Ranking keys
    dfc["_k1"] = dfc["_composite_score"]                                    # 1. Composite score
    dfc["_k2"] = -dfc["AUPRC_active"].astype(float)                        # 2. AUPRC (tie-breaker)
    dfc["_k3"] = -dfc["AUROC"].astype(float)                               # 3. AUROC (tie-breaker)
    dfc["_k4"] = dfc["Brier"].astype(float)                                 # 4. Brier (tie-breaker)
    
    return dfc.sort_values(["_k1", "_k2", "_k3", "_k4"]).drop(columns=["_k1", "_k2", "_k3", "_k4", "_ci_width", "_composite_score"])
...
def run_qsar_evaluation(config: dict, log: logging.Logger):
    """
    Run the QSAR evaluation pipeline with activity cliff-aware metrics.
    
    This function loads data, builds features, trains models and writes all results.
    """
    ...
    # Use Cheminformatics thresholds (consistent with AC_Analysis and ReverseQSAR)
    chem_cfg = config.get("Cheminformatics", {})
    sim_thr = float(chem_cfg.get("similarity_threshold", 0.85))
    dp_thr = float(chem_cfg.get("potency_diff_threshold", 1.0))
    ...
    for backbone in backbones:
        ...
        # ALWAYS save selected feature names (for generator scorer) - regardless of variance filtering
        # This ensures scorer can load features for any backbone (descriptors, MACCS, ECFP)
        try:
            selected_features_path = RR / "selected_features.txt"
            with open(selected_features_path, "w") as f:
                f.write("\n".join(feat_names_used))
            log.info(f"[Features] Saved {len(feat_names_used)} selected features to {selected_features_path.name}")
        except Exception as e:
            log.warning(f"[Features] Failed to save selected_features.txt: {e}")
        ...
        # Save CSV files: numeric (calculations) + formatted (publication) + all_runs (for diagnostics)
        if all_runs_metrics:
            # Save all individual runs
            runs_csv = RR / "model_metrics_all_runs.csv"
            pd.DataFrame(all_runs_metrics).to_csv(runs_csv, index=False, float_format='%.3f')
        ...
        stability_weight = float(config.get("QSAR_Eval", {}).get("stability_weight", 1.0))
        # Use aggregated metrics (mean over all repeats) with composite score selection
        ranked = _rank_models(pd.DataFrame(aggregated_metrics), primary_metric, stability_weight)
        best_row = ranked.iloc[0]
        best_name = str(best_row['model'])
        
        # Report selection
        ci_width = best_row.get(f"{primary_metric}_CI_upper", 0) - best_row.get(f"{primary_metric}_CI_lower", 0)
        mean_val = best_row.get(primary_metric, 0)
        composite_score = mean_val + (stability_weight * ci_width)
        print(f"  🏆 Best model for {backbone}: {best_name} (composite score={composite_score:.3f}: {primary_metric}={mean_val:.3f} + {stability_weight}×CI_width={ci_width:.3f})")
    ...
    if non_empty:
        ...
        # Use numeric for ranking
        all_rows = all_rows_numeric

        # Figures are generated in R
        primary_metric = str(qcfg.get("primary_metric", "RMSE_cliff"))
        stability_weight = float(qcfg.get("stability_weight", 1.0))
        ranked_overall = _rank_models(all_rows, primary_metric, stability_weight)
        best_overall = ranked_overall.iloc[0]

        b_tag = str(best_overall["backbone"])
        m_name = str(best_overall["model"])
        
        # Log selection (by composite score balancing mean and stability)
        ci_lower = best_overall.get(f"{primary_metric}_CI_lower", float("nan"))
        ci_upper = best_overall.get(f"{primary_metric}_CI_upper", float("nan"))
        ci_width = ci_upper - ci_lower if not np.isnan(ci_upper) and not np.isnan(ci_lower) else 0.0
        mean_val = best_overall.get(primary_metric, float("nan"))
        composite_score = mean_val + (stability_weight * ci_width) if not np.isnan(mean_val) else float("nan")
        print(f"🏆 Best OVERALL model → {m_name} [{b_tag}]")
        print(f"   Composite score: {composite_score:.3f} = {primary_metric}={mean_val:.3f} + ({stability_weight}×CI_width={ci_width:.3f})")
